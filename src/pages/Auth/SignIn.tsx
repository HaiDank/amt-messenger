import React, { useEffect } from 'react';
import { GoogleAuthProvider, signInWithPopup } from 'firebase/auth';
import { FaFacebookMessenger } from 'react-icons/fa';
import { FcGoogle } from 'react-icons/fc';
import { useNavigate } from 'react-router-dom';
import { useAppDispatch, useAppSelector } from '../../hooks/useAppRedux';
import { setUser, updateUserStatus } from '../../state/userSlice';
import { auth } from '../../config/firebase';

const SignIn: React.FC = () => {
	const dispatch = useAppDispatch();
	const navigate = useNavigate();
	const user = useAppSelector(state => state.user);
	const signInWithGoogle = () => {
		const provider = new GoogleAuthProvider();
		signInWithPopup(auth, provider)
			.then((result) => {
				const name = result.user.displayName!;
				const avatarUrl = result.user.photoURL!;
				const uid = result.user.uid;

				dispatch(setUser({name, uid, avatarUrl}))
				dispatch(updateUserStatus());
				console.log('usersignin', user)
				navigate('/chats', { replace: true });
				return;
			})
			.catch((error) => {
				const errorCode = error.code;
				const errorMessage = error.message;
				console.error(errorCode, errorMessage);
			});
	};

	useEffect(()=> {
		if ( user.online ){
			navigate('/chats', { replace: true });
		}
	}, [])

	const MessengerIcon = () => {
		return (
			<svg
				xmlns='http://www.w3.org/2000/svg'
				width='100%'
				height='100%'
				fill='none'
				viewBox='0 0 48 48'
				id='messenger'
			>
				<path
					fill='url(#paint0_radial_147648_891)'
					d='M23.9979 0C10.4811 0 0 9.90512 0 23.2779C0 30.2733 2.86775 36.3208 7.53533 40.4964C7.9253 40.8444 8.16528 41.3363 8.17728 41.8643L8.30926 46.1359C8.35126 47.4978 9.75514 48.3857 11.003 47.8338L15.7666 45.7339C16.1686 45.554 16.6245 45.524 17.0505 45.638C19.2403 46.2379 21.5681 46.5619 23.9979 46.5619C37.5147 46.5619 47.9957 36.6568 47.9957 23.2839C47.9957 9.91112 37.5147 0 23.9979 0Z'
				></path>
				<path
					fill='#fff'
					d='M9.58715 30.0873L16.6365 18.9043C17.7584 17.1225 20.1582 16.6845 21.8441 17.9444L27.4536 22.15C27.9695 22.534 28.6775 22.534 29.1874 22.144L36.7587 16.3965C37.7667 15.6286 39.0865 16.8405 38.4146 17.9144L31.3592 29.0914C30.2373 30.8732 27.8375 31.3112 26.1517 30.0513L20.5422 25.8457C20.0262 25.4617 19.3183 25.4617 18.8083 25.8517L11.237 31.5992C10.2291 32.3671 8.90921 31.1612 9.58715 30.0873Z'
				></path>
				<defs>
					<radialGradient
						id='paint0_radial_147648_891'
						cx='0'
						cy='0'
						r='1'
						gradientTransform='matrix(52.2962 0 0 52.2961 9.24 47.734)'
						gradientUnits='userSpaceOnUse'
					>
						<stop stopColor='#09F'></stop>
						<stop offset='.61' stopColor='#A033FF'></stop>
						<stop offset='.935' stopColor='#FF5280'></stop>
						<stop offset='1' stopColor='#FF7061'></stop>
					</radialGradient>
				</defs>
			</svg>
		);
	};

	return (
		<div className='w-full h-full flex flex-col items-center justify-between pt-40 pb-20 relative bg-[#333]'>
			<span className='absolute text-white left-2 top-2 drop-shadow-md'>
				<FaFacebookMessenger />
			</span>
			<div className='flex flex-col items-center gap-4'>
				<span className='w-16 aspect-square drop-shadow-md '>
					<MessengerIcon />
				</span>
				<p className='text-3xl font-bold text-white drop-shadow-md'>
					Welcome to Messenger
				</p>
				<p className='text-lg font-bold text-neutral-500'>
					The simple way to text, call and video chat right from your
					desktop.
				</p>
			</div>
			<div className='flex flex-col items-center w-1/3'>
				<button
					onClick={signInWithGoogle}
					className='flex items-center justify-center w-full h-10 gap-4 font-semibold bg-white rounded-md shadow-md text-neutral-600 hover:bg-neutral-100 focus:bg-neutral-300'
				>
					<span className='text-xl'>
						<FcGoogle />
					</span>
					Sign In with Google
				</button>
			</div>
		</div>
	);
};

export default SignIn;
